which gcloud > /dev/null 2>&1 || return

function gcpx() {
    # xx {"tags": "gcp", "description": "Activate GCP Account for shell", "subshell": true, "cache": false}
    local BIND_OPTIONS=()
    local TEXT_PROMPT="CTRL+O: Open Console"
    if [[ ! -n ${XX_CALLBACK_FROM_TMUX} ]]; then
        BIND_OPTIONS+="--bind=enter:become(echo {})+rebind(enter)"
        TEXT_PROMPT="ENTER: Activate Account"
    else
        BIND_OPTIONS+="--bind=start:unbind(enter)"
    fi
    local SELECTED_ACCOUNT=$(__xx_construct_google_cloud_sdk_accounts | fzf --exact --ansi --header-lines=1 --info=inline \
        --prompt="> Filter " \
        --layout=reverse-list \
        --border-label ' GCP Accounts ' --color 'border:#a6e3a1,label:#a6e3a1,header:#a6e3a1:bold,header:#a6e3a1' \
        --header "${TEXT_PROMPT}" --tmux 40% \
        --bind="ctrl-o:execute-silent(${XX_OPEN_COMMAND} 'http://console.cloud.google.com')" \
        ${BIND_OPTIONS[@]}
    )
    if [ ! -z ${SELECTED_ACCOUNT} ]; then
        export CLOUDSDK_CORE_ACCOUNT="${SELECTED_ACCOUNT}"
        export XX_CLOUDSDK_ACTIVE="true"
    fi
}
