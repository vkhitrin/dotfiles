...
function tdx() {
    ...
    local BIND_OPTIONS=()
    local TEXT_PROMPT="CTRL+R: Refresh | CTRL+E: Edit Task | CTRL+N: New Task | CTRL+P: Toggle Preview"
    ...
    if [ ! -z "${TMUX}" ]; then
     ...
     __xx_get_todo | fzf --border-label " TODO " \
        ...
        --preview 'source ~/.zshrc.d/xx_functions/__xx_preview_todo; __xx_preview_todo {-1}' \
        --header-lines=1 \
        --layout=reverse-list \
